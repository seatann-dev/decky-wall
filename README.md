...
1. On the **desktop side** (Bazzite desktop, or any PC with your WE wallpapers), open `video-wallpaper-gallery.html` in Chrome/Edge.
...
The plugin is TypeScript/React built with the Decky toolchain (`@decky/api`, `@decky/ui`, `@decky/rollup`). `pnpm i && pnpm build` produces `dist/index.js`. The selector (`video-wallpaper-gallery.html`) is a single self-contained file — no build step.
